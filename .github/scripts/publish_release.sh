#!/usr/bin/env bash
# Zerium Browser — release publisher.
#
# Responsibilities:
...
- In-stream video ads (including YouTube's) are suppressed client-side — player-API pruning, auto-skip, overlay hiding — not removed at network level; some formats can still slip through as sites change. Scope and rationale: [docs/CONTENT_BLOCKING.md]({REPO_URL}/blob/main/docs/CONTENT_BLOCKING.md).
- No extension support (System WebView engine — deliberate tradeoff, see [docs/ARCHITECTURE.md]({REPO_URL}/blob/main/docs/ARCHITECTURE.md)).
